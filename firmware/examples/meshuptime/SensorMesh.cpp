...
mesh::Packet* SensorMesh::createSelfAdvert() {
  ...
  uint8_t app_data_len = _cli.buildAdvertData(self_adv_type, app_data);
...
void SensorMesh::handleCommand(uint32_t sender_timestamp, char* command, char* reply) {
  ...
  } else if (strcmp(command, "get adv.type") == 0) {
    sprintf(reply, "%d", (int)self_adv_type);
  } else if (memcmp(command, "set adv.type ", 13) == 0) {
    const char* v = &command[13];
    while (*v == ' ') v++;
    /* 1 = chat, 4 = sensor. De andere types (repeater/room) horen bij een node
     * die dat werkelijk is en zou verwarrend zijn hier, dus alleen deze twee. */
    if (*v == '1' || *v == '4') {
      setSelfAdvType((uint8_t)(*v - '0'));
      sprintf(reply, "OK - adv.type=%d (%s)", (int)self_adv_type,
              self_adv_type == 1 ? "chat" : "sensor");
    } else {
      strcpy(reply, "Err - 1 (chat) of 4 (sensor)");
    }
...
void SensorMesh::onAnonDataRecv(mesh::Packet* packet, const uint8_t* secret, const mesh::Identity& sender, uint8_t* data, size_t len) {
  if (packet->getPayloadType() == PAYLOAD_TYPE_ANON_REQ) {  // received an initial request by a possible admin client (unknown at this stage)
    ...
    if (packet->isRouteFlood()) {
      ...
      if (path) floodScoped(path, SERVER_RESPONSE_DELAY, packet->getPathHashSize());
    } else {
      mesh::Packet* reply = createDatagram(PAYLOAD_TYPE_RESPONSE, sender, secret, reply_data, reply_len);
      if (reply) floodScoped(reply, SERVER_RESPONSE_DELAY, packet->getPathHashSize());
    }
  }
}
...
#define ADV_TYPE_PATH   "/adv_type"
void SensorMesh::loadAdvType() {
  self_adv_type = 1;   // chat als standaard
  if (_fs == NULL || !_fs->exists(ADV_TYPE_PATH)) return;
#if defined(RP2040_PLATFORM)
  File f = _fs->open(ADV_TYPE_PATH, "r");
#else
  File f = _fs->open(ADV_TYPE_PATH);
#endif
  if (!f) return;
  char c = 0;
  if (f.read((uint8_t *) &c, 1) == 1 && (c == '1' || c == '4')) self_adv_type = c - '0';
  f.close();
}
void SensorMesh::saveAdvType() {
  if (_fs == NULL) return;
  File f = _fs->open(ADV_TYPE_PATH, "w");
  if (!f) return;
  uint8_t c = '0' + self_adv_type;
  f.write(&c, 1);
  f.close();
}
void SensorMesh::setSelfAdvType(uint8_t t) {
  if (t != 1 && t != 4) return;
  if (t == self_adv_type) return;
  self_adv_type = t;
  saveAdvType();
  /* Meteen een advert, anders blijft de oude soort in andermans lijst staan tot
   * het volgende geplande advert. */
  sendSelfAdvertisement(2000, false);
}
...
void SensorMesh::sendAckTo(const ClientInfo& dest, uint32_t ack_hash, uint8_t path_hash_size) {
  if (dest.out_path_len == OUT_PATH_UNKNOWN) {
    ...
    if (ack) floodScoped(ack, TXT_ACK_DELAY, path_hash_size);
  } else {
      ...
      if (packet->isRouteFlood()) {
        ...
        if (reply) {
          if (from->out_path_len != OUT_PATH_UNKNOWN) {  // we have an out_path, so send DIRECT
            ...
            floodScoped(reply, SERVER_RESPONSE_DELAY, packet->getPathHashSize());
          }
        }
      }
    } else {
      ...
      if (flags == TXT_TYPE_PLAIN) {
        ...
        if (handled) { // if msg was handled then send an ack
          ...
          if (packet->isRouteFlood()) {
            ...
            if (path) floodScoped(path, TXT_ACK_DELAY, packet->getPathHashSize());
          } else {
        ...
        if (text_len > 0) {
          ...
          if (reply) {
            if (from->out_path_len == OUT_PATH_UNKNOWN) {
              floodScoped(reply, CLI_REPLY_DELAY_MILLIS, packet->getPathHashSize());
            } else {
