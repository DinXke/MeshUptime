...
// add hardware serial interface
  ...
  // MeshUptimeCompanion: bring up buzzer, accelerometer, persisted config and the
  // alert/command engine. Done here so identity, prefs and the mesh interface are
  // already live and DMs are already being received by the stack.
  mu_begin();
...
void loop() {
  ...
  sensors.loop();
  mu_loop();   // MeshUptimeCompanion: buzzer/LED, button, serial CLI, fall detection
#ifdef DISPLAY_CLASS
  ui_task.loop();
#endif
  rtc_clock.tick();
  ...
  // MeshUptimeCompanion: never light-sleep while a tune/LED pattern is playing, the
  // find melody is looping, or a fall pre-alarm is counting down — those need a
  // steadily-running loop for smooth audio and a responsive button.
  if (!the_mesh.hasPendingWork() && !mu_wants_cpu()) {
